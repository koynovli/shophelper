...
| Шаг | Действие | UI / API |
|-----|----------|----------|
| 1 | **Номенклатура** — карточка Product (SKU, габариты, категория, допустимые типы оборудования) | Админка → вкладка **Номенклатура**; `POST /api/products/` (admin) |
...
Единый профиль типа — `core/equipment_profiles.py` (зеркало на фронте: `frontend/src/map/equipmentProfiles.ts`):

| type | layout | слоты по умолчанию | полки Shelf |
|------|--------|-------------------|-------------|
| `shelf`, `fridge` | сетка 4×N | `rows_count × 4` | да |
| `hanger` | linear | 1–2 рейла на всю ширину | да (низкие) |
| `box` | single | 1 слот | да (ёмкость) |
| `mannequin` | expo_vertical | 3 зоны (верх / низ / аксессуар) | **нет** |

Манекен: `max_capacity = 1` на зону; вместимость считается по виртуальным габаритам без `Shelf` в БД.

**Индикация слотов** (режим мерчандайзинга, DOM): каждый `EquipmentSlot` — отдельная ячейка по `layout_mode`; цвет по `current_qty / max_capacity`:

- зелёный — &gt; 70%
- жёлтый — 30–70% или активная задача выкладки
- красный — &lt; 30%

Поля слота в API: `slot_label`, `active_placement_task`, `nearest_batch_expiry` (FEFO по партиям на полке). Spatial Engine считает `max_capacity` по типу оборудования (сетка / вешалка / навал / манекен = 1).

UI: вкладка **Карта зала** — в режиме мерчандайзинга DOM-слоты + правая панель (фильтр товаров по `allowed_equipment_types`); в режиме редактирования — DOM-перетаскивание оборудования.
