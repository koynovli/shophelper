...
/** Имя вида «Стеллаж N», N уникален среди уже существующих объектов этого типа в текущем состоянии карт. */
function nextAutogeneratedEquipmentName(zones: FloorZone[], type: FloorEquipmentType): string {
  const normalizedType = type;
  const label = EQUIPMENT_TYPE_LABEL_RU[normalizedType];
  const re = new RegExp(`^${escapeRegExp(label)}\\s+(\\d+)$`, 'u');
  let maxN = 0;
  for (const z of zones) {
    for (const eq of z.equipment) {
      if (normalizeEquipmentTypeValue(String(eq.type)) !== normalizedType) {
        continue;
      }
      const m = re.exec(eq.name.trim());
      if (m) {
...
function StoreMap() {
  ...
  /** Последняя позиция указателя на карте (см, top-left нового объекта 120×60), для E / кнопки быстрого создания */
  const lastCreatePointerCmRef = useRef<{ x: number; y: number } | null>(null);
  ...
  const updateQuickCreatePointerFromClient = useCallback(
    (clientX: number, clientY: number): void => {
      const board = mapBoardRef.current;
      if (!board || mapWidthPx <= 0 || mapHeightPx <= 0) {
        return;
      }
      ...
      if (
        ...
        lastCreatePointerCmRef.current = null;
        return;
      }
      let offsetX = ((clientX - rect.left) / rect.width) * mapWidthPx;
      let offsetY = ((clientY - rect.top) / rect.height) * mapHeightPx;
      offsetX = snapCoordPx(offsetX);
      offsetY = snapCoordPx(offsetY);
      const placeholder: Pick<FloorEquipment, 'width' | 'height'> = { width: 120, height: 60 };
      lastCreatePointerCmRef.current = clampEquipmentTopLeftCm(
        placeholder,
        offsetX / PX_PER_CM,
        offsetY / PX_PER_CM,
      );
    },
    [clampEquipmentTopLeftCm, mapHeightPx, mapWidthPx],
  );
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent): void => {
      updateQuickCreatePointerFromClient(event.clientX, event.clientY);
    };
    window.addEventListener('pointermove', onMove, { passive: true });
    return () => window.removeEventListener('pointermove', onMove);
  }, [editMode, updateQuickCreatePointerFromClient]);

  const getQuickCreateCoordsCm = useCallback((): { x: number; y: number } => {
    const fromPointer = lastCreatePointerCmRef.current;
    if (fromPointer) {
      return { x: fromPointer.x, y: fromPointer.y };
    }
    ...
    if (!vp || !board || mapWidthPx <= 0 || mapHeightPx <= 0) {
      return clampEquipmentTopLeftCm(placeholder, fallback.x, fallback.y);
    }
    ...
    let offsetX = ((cx - br.left) / br.width) * mapWidthPx;
    let offsetY = ((cy - br.top) / br.height) * mapHeightPx;
    offsetX = snapCoordPx(offsetX);
    offsetY = snapCoordPx(offsetY);
    const xCm = offsetX / PX_PER_CM;
    const yCm = offsetY / PX_PER_CM;
    return clampEquipmentTopLeftCm(placeholder, xCm, yCm);
  }, [clampEquipmentTopLeftCm, mapHeightPx, mapWidthPx]);
  ...
  const quickCreateShelving = useCallback(async (): Promise<void> => {
    const zoneId = zonesRef.current[0]?.id ?? null;
    if (!zoneId) {
      alert('Нет доступной зоны для добавления оборудования.');
      return;
    }
    const { x, y } = getQuickCreateCoordsCm();
    const name = nextAutogeneratedEquipmentName(zonesRef.current, 'shelving');
    const payload = {
      name,
      zone: zoneId,
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        setZones((prevZones) =>
          prevZones.map((zone) =>
            zone.id === zoneId ? { ...zone, equipment: [...zone.equipment, created] } : zone,
          ),
        );
      ...
      console.error('Ошибка быстрого создания оборудования:', error);
      alert('Не удалось создать оборудование. Проверьте консоль.');
    } finally {
      setIsSaving(false);
    }
  }, [getQuickCreateCoordsCm, pushUndoSnapshot]);
  ...
  }, [cloneSelected, editMode, quickCreateShelving, redo, rotateSelected, undo]);
  ...
  return (
    <section className="flex min-h-0 w-full flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-700/80 bg-slate-900/90 px-3 py-2 text-sm text-slate-300 shadow-lg backdrop-blur-sm">
        ...
        <button
          type="button"
          disabled={!editMode || isSaving || !defaultZoneId}
          onClick={() => {
            void quickCreateShelving();
          }}
          className="inline-flex items-center gap-2 rounded-lg border border-slate-600 bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-300 transition hover:border-emerald-500/50 hover:text-emerald-100 disabled:cursor-not-allowed disabled:opacity-50"
          title={
            !editMode
              ? 'Включите режим редактирования'
              : !defaultZoneId
                ? 'Нет зоны на карте'
                : 'Создать оборудование (E)'
          }
        ...
        <div className="pointer-events-none absolute bottom-3 left-4 z-20 max-w-[min(100%,28rem)] rounded-lg border border-slate-700/80 bg-slate-950/85 px-3 py-2 text-[11px] text-slate-400 backdrop-blur-sm">
          ...
          {editMode ? (
            <span className="mt-1 block text-amber-200/90">
              Клик по карте — новый объект · R / Shift+R — поворот · D — клон · E — стеллаж под курсором
              (иначе центр вида) · Ctrl+Z / Ctrl+Y — отмена / повтор
            </span>
          ) : null}
        </div>
      </div>
